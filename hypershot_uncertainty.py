...
EPS = 0.001 # Value to improve visibility on the boxplot since variance on seen data is so low.
...
def experiment(N):
    ...
    for _ in range(N):
        ...
        sample = torch.nn.functional.softmax(o).clone().data.cpu().numpy()
        for i in range(model.n_way):
            R2[i].append(sample[i] + EPS*np.random.normal() * (1 if sample[i] < 0.5 else -1))
